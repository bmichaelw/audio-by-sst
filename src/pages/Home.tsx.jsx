...
export default function Home() {
  ...
  const themeNames = useMemo(() => themesData.map((t: any) => t.name), [themesData]);
    ...
    return tracks.filter((track: any) => {
      ...
      if (filters.search) {
        ...
        const matchesSearch =
          ...
          track.themes?.some((t: string) => t.toLowerCase().includes(searchLower)) ||
          track.tags?.some((t: string) => t.toLowerCase().includes(searchLower));
  ...
  const featuredTracks = useMemo(() => {
    return tracks.filter((t: any) => t.is_featured).slice(0, 4);
  }, [tracks]);
